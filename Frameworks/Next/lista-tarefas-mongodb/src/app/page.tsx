...
export default function Home() {
  ...
  const updateTarefa = async (id: string, statusTarefa: boolean) => {
    try {
      ...
      if (data.success) {
        ...
        // setTarefas( tarefas.map((tarefa) => (tarefa.id == id ? data.data : tarefa)) ) //cliente-side
      }
    } catch (error) {
      console.error(error)
  ...
  const deleteTarefa = async (id: string) => {
    try {
      ...
      if (resultado) {
        setTarefas(tarefas.filter((tarefa) => tarefa._id !== id))// remove a terefa do vector
      }
    } catch (error) {
      console.error(error);
    }
      ...
      <ul>
        {tarefas.map((tarefa) => (
          <li key={tarefa._id.toString()}>
            {tarefa.titulo}
            <input type="checkbox" checked={tarefa.concluida} onChange={()=>updateTarefa(tarefa.id.toString(), tarefa.concluida)} />
